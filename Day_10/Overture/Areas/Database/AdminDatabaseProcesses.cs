...
namespace Overture.Areas.Database
{
    public static class AdminDatabaseProcesses
    {
        ...
        public static List<AdminManageModel> AddMovie(string userName, string movieName, string category)
        {
            var resutModel = new List<AdminManageModel>();
        ...
        public static bool Delete(Movie incominMovie)
        {
            bool result = false;

            using (var DB = new OvertureContext())
            {
                var findMovie = DB.Movies.Where(f => f.Name == incominMovie.Name).Include(c => c.Categories).FirstOrDefault();
